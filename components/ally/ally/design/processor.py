...
class Processor:
    ...
    def register(self, processing):
        ...
        processing._calls.append(self.call)
...
class Chain:
    ...
    def __init__(self, processing):
        ...
        '''
        if isinstance(processing, Processing): processing = processing._calls
    ...
    def branch(self, processing):
        ...
        assert isinstance(processing, Processing), 'Invalid processing %s' % processing
        self._calls.clear()
        self._calls.extend(processing._calls)
        self._proceed = True
        return self
...
class AssemblyAttribute:
    ...
    def create(self):
        ...
        if self.defined and self.required:
            ...
            if not types:
                ...
                raise AssemblyError('Invalid defined %s for required %s assembly types, based on attributes:%s' % tuple(args))
    ...
    def add(self, attribute):
        ...
        if self.required and self.defined and self.required != self.defined and self.defined.issuperset(self.required):
                args = []
                args.append(attribute.name)
                args.append(', '.join('\'%s\'' % typ.__name__ for typ in self.defined))
                args.append(', '.join('\'%s\'' % typ.__name__ for typ in self.required))
                argss = ['%s\n, of attribute %s' % (locationStack(attr.clazz), attr) for attr in self.attributes]
                argss.append('%s\n, of attribute %s' % (locationStack(attribute.clazz), attribute))
                args.append(''.join(argss))
                raise AssemblyError('The defined attribute \'%s\' types %s are not compatible with the required types %s'
                                    '\nCurrent assembly is based on attributes:%s' % tuple(args))
